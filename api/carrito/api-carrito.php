...
if(isset($_GET['action'])){
    ...
    echo json_encode(['statuscode' => 404, 
                        'response' => 'No se puede procesar la solicitud 1']);
}
...
function mostrar($carrito){
    ...
    foreach($itemsCarrito as $itemCarrito){
        $httpRequest = file_get_contents('http://localhost/CARRITO/api/productos/api-productos.php?get-item=' . $itemCarrito['id']); 
